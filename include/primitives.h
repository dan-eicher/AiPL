...
void fn_intersection(Machine* m, Value* axis, Value* lhs, Value* rhs);  // ∩ dyadic (intersection)
void fn_find(Machine* m, Value* axis, Value* lhs, Value* rhs);           // ⍷ dyadic (find)
...
extern PrimitiveFn prim_intersect; // ∩ symbol (intersection)
extern PrimitiveFn prim_find;      // ⍷ symbol (find - epsilon-underbar)
